 ...
 *
 ...
 9) test_connectivity2   pass: true
 10) test_connectivity2   pass: true
 11) test_connectivity_E   pass: true
 12) test_n(a)   pass: true
 13) test_n(b)   pass: true
 14) test_n(a)   pass: true
 15) test_n(b)   pass: true
 16) test_connectivity0   pass: true
 17) test_connectivity1   pass: true
 18) test_connectivity2   pass: true
 19) test_path   pass: true
 20) runtime test (9):   pass: true
 21) runtime test (9):   pass: true
 22) runtime test (12):   pass: true
 number of Errors: 0 of 23 tests, 0 exceptions, time: 6.032 seconds
 grade: 100
...
public class Test {
    ...
    public static void main(String[] args) {
        ...
        try{test_large(5000,100*5000,2);}
        catch (Exception e) {e.printStackTrace(); _number_of_exception++;}
        ...
        boolean t = dt<7;
        test("runtime test (9): ",t, true);
        t = dt<9;
        test("runtime test (9): ",t, true);
        t = dt<12;
        test("runtime test (12): ",t, true);
    ...
    public static void test_large(int size, int edge, int seed ) {
        graph g0 = graph_creator(size, edge, seed);
        graph_algorithms ga = new Graph_Algo();
        ga.init(g0);
        graph g = ga.copy();
        ga.init(g);
        int[] nodes = nodes(g);
        while(g.edgeSize()>edge*0.9) {
            int a = nextRnd(0,size);
            int b = nextRnd(0,size);
            int ai = nodes[a], bi = nodes[b];
            g.removeEdge(ai, bi);
        }
        System.out.println(g0);
        boolean b = ga.isConnected();
        test("test_connectivity2 ",b, true);
        g.addNode(new NodeData());
        b = ga.isConnected();

        test("test_connectivity2 ",b, false);
        test("test_connectivity_E ", g0.edgeSize(), edge);
    }
    /**
